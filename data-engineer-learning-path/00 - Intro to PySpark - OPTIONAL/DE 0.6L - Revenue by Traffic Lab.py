...
# TODO
chain_df = ((df.groupby("traffic_source").agg(sum("revenue").alias("total_rev"), avg("revenue").alias("avg_rev"))).withColumn("avg_rev", round("avg_rev",2)).withColumn("total_rev", round("total_rev",2))).sort(col("total_rev").desc()).limit(3)
